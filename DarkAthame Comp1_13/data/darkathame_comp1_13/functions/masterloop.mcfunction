#Use Hashtags To Enable/Disable Functions
#Ex Of Disabled: #function example2:tester/test1 
#Ex Of Enabled: function example2:tester/test1 

##Diamond Man
function darkathame_comp1_13:diamondman/diamondman

##Dragon Eat Deafening Roar
function darkathame_comp1_13:dragoneatdeafeningroar/deafroar

##Walking Experience
#function darkathame_comp1_13:walkingexperience/walkxp

##Glowing Chickens
function darkathame_comp1_13:glowingchickens/chickenglow

##Freeze/Invulnrable All Entites
#function darkathame_comp1_13:18w30bfreeze/frozeninvuln

##Dragon Sword
#function darkathame_comp1_13:dragonsword/dragonsword

##Random Entity Fall-Through
#function darkathame_comp1_13:worldfallthough/fallthrough

##Embiggening Phantoms
#function darkathame_comp1_13:embiggeningphantoms/phantomenlarge

##Nitwit Only Mode
#function darkathame_comp1_13:nitwitonlymode/nitwit

##Best Creeper Ideas
#function darkathame_comp1_13:bestcreeperidea/bestcreeper

##Netherwart Contagion
function darkathame_comp1_13:netherwartcontagion/netherwarts

##Overworld Exploding Beds
function darkathame_comp1_13:overworldexplodebeds/explodingbeds

##Fire Fighter Golems
function darkathame_comp1_13:firefightergolem/firefgolem

##Explosive Pigs
function darkathame_comp1_13:explosivepigs/explodingpig

##The Wight
function darkathame_comp1_13:stalkingwight/wight

##Cheeto
function darkathame_comp1_13:cheetos/cheeto

##Lit Creepers
#function darkathame_comp1_13:litcreepers/excreeper

##Tide Pods
function darkathame_comp1_13:tidepod/edibletide

##Cross Breeding
function darkathame_comp1_13:crossbreeding/crossbreed

##Creeper Conga
function darkathame_comp1_13:creeperconga/creeperconga
